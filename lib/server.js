...
var jojo = function(httpServer){
    ...
    this.server = null;
    if(httpServer)
        this.server = io(httpServer,{path : '/socket'});
    else
        this.server = io(config.get("socket.port"));
    this.io = this.server;
...
jojo.prototype.controller = function(regex){
    ...
    if(!this.controllers.hasOwnProperty(regex)){
        this.controllers[regex] = new Controller(this.server,regex);
    }
    return this.controllers[regex];
};

jojo.prototype.auth = function(guard){
    this.server.use(function(socket,next){
