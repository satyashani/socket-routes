...
var jojo = function(httpServer){
    ...
    var onJoin = function(socket,message){
        handler.call(self,socket,events.join,message);
    };
    var onLeave = function(socket,message){
        handler.call(self,socket,events.leave,message);
    };
    server.on('connection',function(socket){
        if(!socket.request.headers.cookie){
            ...
            socket.on(events.join,function(message){
                onJoin.call(self,socket,message);
            });
            socket.on(events.leave,function(message){
                onLeave.call(self,socket,message);
            });
            socket.on(events.message,function(message){
                handler.call(self,socket,events.message,message);
            });
        }
    });
};
